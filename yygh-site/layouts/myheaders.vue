...
export default {
  ...
  mounted() {
    ...
    // 定义微信回调方法(定义一个方法等待调用)
    // 就是定义一个loginCallback方法，当子页面使用window.parent触发该方法时执行方法体内的内容
    // 这里方法体内容是调用vue组件中method定义的loginCallback方法
    let self = this;
  ...
  methods: {
    loginCallback(name, token, openid){
      ...
      if(openid != null) {
        ...
        // openid等于空说明数据库中有该用户的信息，可以直接登录（将token等信息存入cookie）
    ...
    weixinLogin() {
      ...
      // 初始化微信相关参数
      weixin.getLoginParam().then(resp =>{
        // 准备显示二维码的相关数据
        var obj = new WxLogin({
          self_redirect:true,
          id: 'weixinLogin', // 需要显示的容器id
          appid: resp.data.appid, // 公众号appid wx*******
          scope: resp.data.scope, // 网页默认即可
          redirect_uri: resp.data.redirect_uri, // 授权成功后回调的url
          state: resp.data.state, // 可设置为简单的随机数加session用来校验
